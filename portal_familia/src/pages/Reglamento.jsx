...
function Reglamento() {
  ...
  const obj = {
    ...
    "03": {
      ...
      title: "Reglamento de los laboratorios de cómputo y los servicios de telecomunicaciones"
    },
    "04": {
    ...
    "09": {
      ...
      title: "Protocolo de Atencion para Casos de Violencia de Genero de Prepa Ibero"
    }, 
    "010": {
      file: `${BASICURL}10 Anexo 1 Conceptos sobre Protocolo Casos Violencia de Genero Prepa Ibero.pdf`,
      title: "Anexo 1 Conceptos sobre Protocolo Casos Violencia de Genero Prepa Ibero"
    }, 
    "011": {
      file: `${BASICURL}11 Anexo 2 Glosario Definiciones y Marco Conceptual Protocolo Violencia Genero Prepa Ibero.pdf`,
      title: "Anexo 2 Glosario Definiciones y Marco Conceptual Protocolo Violencia Genero"
    }, 
  }
  const TotalFiles = Object.keys(obj).length
  const [menuReglamento, openMenu] = useState(false)
  const [file, switchFile] = useState("00")
  const Index_format = ["00", "01","02", "03", "04", "05", "06", "07", "08", "09", "010", "011"]; 
  const [index_Display, setIndexDisplay] = useState(Index_format.findIndex(iterator=> iterator === file)+1)
  useEffect(() => {
    setIndexDisplay(Index_format.findIndex(iterator=> iterator === file)+1)
  }, [file]);
  ...
  return (
    <Fragment>
      ...
      <section className="Pdf_viewer">
        ...
        {/* Phone view only */}
        <div className="selectIndexPhone" data-show-menu={menuReglamento}>
          <div className='title-reglamento-phone-conteiner'>
            <span className='title-reglamento-phone'>{obj[file].title}</span>
            <i className="fa-solid fa-x" onClick={()=> openMenu(prev=> !prev)}></i>
          </div>
          <div className='items_conteiner_reglamento' onClick={()=> openMenu(prev=> !prev)}>
          {Object.entries(obj).map((iteration, index) => {
            return (
              <div
                key={`${iteration[1].title} numero ${index}`}
